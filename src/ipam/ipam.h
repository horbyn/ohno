...
public:
  auto init(std::unique_ptr<EtcdClientIf> etcd_client) -> bool;
  ...
  auto getAllIp(std::string_view node_name, std::vector<std::string> &all_ip) -> bool override;
...
private:
  ...
  std::unique_ptr<EtcdClientIf> etcd_client_;
};
